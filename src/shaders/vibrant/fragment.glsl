...
uniform sampler2D graffitiTexture;
...
void main() {
    ...
    newFakeUv = fract(newFakeUv + vec2(uTime/(30.0 + sin(cameraPos.x * 0.1) * 30.0), uTime/(30.0 + sin(cameraPos.x * 0.1) * 30.0)));
    ...
    vec4 texture = texture2D(graffitiTexture, newFakeUv + 0.2 * cnoise(vec4(newFakeUv*10.0, uTime/(30.0 + sin(cameraPos.x * 0.4) * 30.0), 0.0), vec4(5.0)));
